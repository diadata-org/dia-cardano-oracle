...
> **[m2-hardening-implementation-plan.md](m2-hardening-implementation-plan.md)**.
...
- [x] **Contracts.** Done (2026-06-06, `d576163`): new `deposit` validator + `deposit_logic` (anti-skim: a CollectDeposit spend is valid only if the tx consumes the canonical Receiver by NFT and raises its lovelace by ≥ the full swept total). Reuses the Receiver `TopUp` redeemer — **no deployed validator changes**, so the live Mainnet Receiver address is untouched. 7 inline tests; `aiken check` 124/0.
- [x] **CLI.** Done (2026-06-06, `0111dee`): `deposit:address` / `deposit:fund` / `deposit:merge`; `makeDepositValidator`. The emulator protocol flow funds two deposits and merges them, so the deposit validator + merge run through real Plutus evaluation on every CLI test run. Wired into `run-all-cli.sh` (steps 32–35, `dec4248`) for the Preview run.
- [ ] **Feeder daemon automation.** Autonomous sweep: when `snapshotBalances` shows a Receiver below `receiver_balance_low_lovelace`, build + submit the merge **through the serial lane** (so it never races the feeder's own updates on the shared Receiver UTxO). The hook is `refreshBalanceGauges` in `daemon-cmd.ts`; the open work is enqueuing the merge under the lane's in-flight lock (the coalescer is intent-shaped). **Not started** — the CLI `deposit:merge` covers the operational need meanwhile; the contract mechanism is delivered + tested.
