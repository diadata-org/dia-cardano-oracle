...
- [Milestone — Feeder Plan](#milestone--feeder-plan)
  ...
  - [Pending — Milestone 2](#pending--milestone-2)
    ...
    - [Recent contract + decommission work (2026-06-07)](#recent-contract--decommission-work-2026-06-07)
    - [Receiver funding — side-deposit + merge (Option A, from the 2026-06-05 audit)](#receiver-funding--side-deposit--merge-option-a-from-the-2026-06-05-audit)
  - [Pending — Mainnet](#pending--mainnet)
  - [Deferred — M3 / M4](#deferred--m3--m4)
  - [Open — DIA decisions (operational, not code)](#open--dia-decisions-operational-not-code)
...
- [x] **Per-transaction metrics + dashboard.** Done: the feeder counted everything per **symbol/intent** — a batch tx of N pairs incremented the counters N times, so "tx confirmed/failed" actually meant "symbol updates", and there was no batch-size or per-tx-stage latency visibility. Added a per-**transaction** metric family (`transactions_total{outcome}`, `transaction_pairs` batch-size histogram, `tx_pair_membership_total{symbol,outcome}`, and tx-stage latency `tx_processing_to_submission_seconds` / `tx_submission_to_confirmation_seconds` / `tx_end_to_end_seconds`), each emitted **once per tx** via the stateless `isTransactionRepresentative` (first batch member) helper. Condemned/superseded intents (no tx, no fee) are excluded from failed-tx counts via `isNoTransactionFailure` (`NonMonotonicNonce`). New dashboard `feeder-tx.json` ("DIA Cardano Oracle Feeder — Transactions") renders tx-stage latency (p50/p95/p99), confirmed-vs-failed throughput, success ratio, and batch-size distribution; `feeder.json` gained a "Row 2b — Transactions (per tx)" condensed view, its noisy per-symbol panels were aggregated (failures by `error_code`, intents-filtered by `reason`, latency across pairs) and renamed truthfully (`Tx confirmed` → `Symbol updates confirmed`). Template vars reordered to cascade **customer → client → symbol**. Tests: `tx-metrics-helpers.test.ts` (representative + no-tx semantics), `metrics.test.ts` (new metrics registered), `threshold-drift.test.ts` (per-panel grouping + both dashboards' template vars).
- [ ] **New metrics dashboard** (broad families; separate from the tx dashboard above). Still uncovered: per-stage latency (the 5 per-symbol phases), `scanner_rpc_errors_total` + backfill, worker active/queue/pool + task counters, `http_request_duration_seconds`, `component_health`, `recovery_attempts_total`, ingest/sanitation counters, `cron_resubmissions_total`, db ops, plus node/process defaults. Align panel windows with the alert windows (PriceAge/PriceDeviation alert at 10 m; panels are 1 h).
