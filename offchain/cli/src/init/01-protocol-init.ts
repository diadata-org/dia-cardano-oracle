...
function defaultProtocolConfigInput(defaultSigner: string): ProtocolInitConfigInput {
...
export function createProtocolStateArtifact(args: {
  ...
  const configInput =
    args.configInput ?? defaultProtocolConfigInput(walletDefaults.paymentKeyHash);
  const configState = {
...
async function promptForProtocolConfigInput(defaultSigner: string): Promise<ProtocolInitConfigInput> {
  console.error("[preview:protocol:init] Enter the initial Config values.");
  const defaults = defaultProtocolConfigInput(defaultSigner);
...
export async function initializeProtocolState(): Promise<ConfigStateArtifact> {
  ...
  const configInput = await promptForProtocolConfigInput(walletDefaults.paymentKeyHash);
