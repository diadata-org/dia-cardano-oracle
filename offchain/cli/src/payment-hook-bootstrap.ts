...
import { readConfigState, type ConfigStateArtifact } from "./state.js";
...
export async function paymentHookBootstrap(args: {
  ...
  const statePath = path.resolve(args.statePath ?? path.resolve("state/preview/config-bootstrap.json"));
  ...
  const paymentHookBootstrapUtxo = selectBootstrapUtxo(
    walletUtxos,
