...
const DEFAULT_INTENT_EXAMPLE_PATH = path.resolve(
  CURRENT_DIR,
  "../../examples/preview/01-oracle-intent-sign.example.json",
);
...
async function resolvePromptDefaults(statePath?: string): Promise<PromptDefaults> {
  ...
  let exampleIntent: IntentSignInput["intent"] = {
  ...
  try {
    const example = JSON.parse(
      await readFile(DEFAULT_INTENT_EXAMPLE_PATH, "utf8"),
    ) as IntentSignInput;
    domain = {
      name: example.domain.name,
      version: example.domain.version,
      sourceChainId: String(example.domain.sourceChainId),
      verifyingContract: example.domain.verifyingContract,
    };
    exampleIntent = example.intent;
  } catch {
    // Fall back to built-in Preview defaults when the example file is unavailable.
  }
...
async function promptExistingFilePath(): Promise<string> {
  while (true) {
    const value = await promptValue({
      ...
      defaultValue: "./examples/preview/01-oracle-intent-sign.example.json",
    });
    try {
...
export async function signPreviewOracleIntentInteractive(): Promise<ReturnType<typeof signPreviewOracleIntentFromInput>> {
  ...
  return signPreviewOracleIntentFromInput({
    input: JSON.parse(await readFile(path.resolve(inputPath), "utf8")) as IntentSignInput,
  });
}
