#!/usr/bin/env bash
#
# package-m3-evidence.sh — assemble the COMPLETE Milestone 3 evidence pack
...
# concurrent reads).
#
# Output: docs/milestones/evidence/m3-<network>-<run-id>/  (a two-digit suffix
...
# scripts/m3-evidence/ → feeder/scripts/ → feeder/ → offchain/ → repo root
...
MD_FILE="milestone-3-${NETWORK}-evidence.md"
...
OUT_DIR="$REPO_ROOT/docs/milestones/evidence/m3-$NETWORK-$STAMP"
...
echo "[package-m3] state dir: $STATE_DIR"
echo "[package-m3] out dir:   $OUT_DIR"
...
# ---------------------------------------------------------------------------
echo "[package-m3] step 1/6 — copying raw logs"
...
# ---------------------------------------------------------------------------
echo "[package-m3] step 2/6 — dumping sqlite tables"
...
# ---------------------------------------------------------------------------
echo "[package-m3] step 3/6 — snapshotting feeder API"
if curl -fsS --max-time 5 "$API_URL/health/live" >/dev/null 2>&1; then
  ...
  echo "[package-m3]   API reachable — snapshots captured"
else
  echo "[package-m3]   API NOT reachable at $API_URL — skipping snapshots"
...
PANELS=(
  ...
  '18|Tx involving router (5m, by router & outcome)|Metric `sum by (router_id, outcome) (increase(dia_bridge_transaction_router_membership_total[5m]))` — transactions that involved each router per 5-minute window, by outcome. A coalesced batch can mix routers on one shared lane, so this is tx↔router membership (one tx can credit several routers), not a pure tx count.'
  ...
  '21|Intents superseded (5m, by reason)|Metric `sum by (reason) (increase(dia_bridge_intents_superseded_total[5m]))` — intents the feeder declined to submit because a newer one already won on-chain (NonMonotonicNonce): no tx, no fee. Correct no-ops, kept out of the failure counters.'
...
PANELS_TX=(
  ...
  '341|Tx counts per client (confirmed/failed, selected range)|Metric `sum by (client_id, outcome) (increase(dia_bridge_transactions_total[$__range]))` — the real integer count of Cardano transactions per client and outcome over the selected range (not an average). A tx batches one client'"'"'s pairs, so the clean per-unit count is per client.'
  '345|Submission state — now (per client)|Metric `dia_bridge_submission_state{client_id}` (last value): what each client'"'"'s submit pipeline is doing now — 0 idle / 1 building / 2 submitting / 3 awaiting-confirmation.'
  '347|Coalescer state — now (per client)|Metric `dia_bridge_coalescer_state{client_id}` (last value): what each client'"'"'s coalescer is doing now — 0 idle / 1 accumulating / 2 in-flight.'
  '342|Submission state per client (history)|State-timeline of `dia_bridge_submission_state{client_id}` (0 idle / 1 building / 2 submitting / 3 awaiting-confirmation). Serial per batch; shows how long each phase took.'
...
if curl -fsS --max-time 5 "$GRAFANA_URL/api/health" >/dev/null 2>&1; then
  if render_dashboard "$GRAFANA_DASHBOARD_UID" "dia-cardano-oracle-feeder" "$OUT_DIR/dashboards/dashboard-full.png" 2>/dev/null; then
    echo "[package-m3]   full dashboard PNG captured"
    ...
    for entry in "${PANELS[@]}"; do
      ...
      if render_panel "$GRAFANA_DASHBOARD_UID" "$panel_id" "$OUT_DIR/dashboards/panel-$panel_id.png" 2>/dev/null; then
        echo "[package-m3]   panel $panel_id PNG captured"
        DASHBOARDS_MD+=$'\n'"**${panel_title}**"$'\n\n'"![${panel_title}](dashboards/panel-${panel_id}.png)"$'\n\n'"${panel_desc}"$'\n'
      else
        echo "[package-m3]   panel $panel_id PNG FAILED — skipping in report" >&2
      fi
    done
    ...
    if render_dashboard "$GRAFANA_TX_DASHBOARD_UID" "dia-cardano-oracle-feeder-transactions" "$OUT_DIR/dashboards/tx-dashboard-full.png" 2>/dev/null; then
      echo "[package-m3]   tx dashboard PNG captured"
      ...
      for entry in "${PANELS_TX[@]}"; do
        ...
        if render_panel "$GRAFANA_TX_DASHBOARD_UID" "$panel_id" "$OUT_DIR/dashboards/tx-panel-$panel_id.png" 2>/dev/null; then
          echo "[package-m3]   tx panel $panel_id PNG captured"
          DASHBOARDS_MD+=$'\n'"**${panel_title}**"$'\n\n'"![${panel_title}](dashboards/tx-panel-${panel_id}.png)"$'\n\n'"${panel_desc}"$'\n'
        else
          echo "[package-m3]   tx panel $panel_id PNG FAILED — skipping in report" >&2
        fi
      done
    else
      echo "[package-m3]   tx dashboard render FAILED — skipping in report" >&2
    fi

    # Internals dashboard (feeder-internals.json, uid dia-cardano-feeder-internals)
    # — the broad metrics view: pipeline-phase latency, scanner, worker pool, DB,
    # cron/recovery. A full render is enough here (troubleshooting surface).
    if render_dashboard "dia-cardano-feeder-internals" "dia-cardano-oracle-feeder-internals" "$OUT_DIR/dashboards/internals-dashboard-full.png" 2>/dev/null; then
      echo "[package-m3]   internals dashboard PNG captured"
      ...
      echo "[package-m3]   internals dashboard render FAILED — skipping in report" >&2
    fi
  else
...
# ---------------------------------------------------------------------------
echo "[package-m3] step 5/6 — computing stats"
...
# ---------------------------------------------------------------------------
echo "[package-m3] step 5b — capturing alert state + remediation"
...
if curl -fsS --max-time 5 "$PROM_URL/api/v1/alerts" -o "$ALERTS_ACTIVE_JSON" 2>/dev/null; then
  echo "[package-m3]   captured live alert state from Prometheus"
else
...
# ---------------------------------------------------------------------------
echo "[package-m3] step 5c — capturing push-policy config"
...
# ---------------------------------------------------------------------------
echo "[package-m3] step 5d — running + capturing test suites"
...
echo "[package-m3]   feeder: $feeder_result ($stat_feeder_pass/$stat_feeder_tests, $stat_feeder_suites suites) — cli: $cli_result"

# ---------------------------------------------------------------------------
# Step 5e — per-feed sanity check (on-chain value vs latest DIA source).
# The accuracy section: per-feed price/timestamp vs the DIA source. Read-only;
# best-effort — it needs
# the Cardano chain + DIA registry reachable, so it degrades to a note offline.
# ---------------------------------------------------------------------------
echo "[package-m3] step 5e — per-feed sanity (accuracy)"
SANITY_DIR="$OUT_DIR/sanity"
mkdir -p "$SANITY_DIR"
if SANITY_OUT_DIR="$SANITY_DIR" CARDANO_NETWORK="$NETWORK_DISPLAY" \
     bash -c "cd '$REPO_ROOT/offchain/feeder' && npm run --silent sanity:feeds" \
     > "$OUT_DIR/sanity/sanity-run.log" 2>&1 && [ -f "$SANITY_DIR/feed-sanity.md" ]; then
  echo "[package-m3]   feed sanity captured"
  SANITY_MD="$(cat "$SANITY_DIR/feed-sanity.md")"
else
  echo "[package-m3]   feed sanity unavailable (chain/registry not reachable) — noting in report" >&2
...
# ---------------------------------------------------------------------------
# Step 5f — alert-trigger logs. Fold in a bundle produced by
# scripts/monitoring/trigger-alert-demo.sh (set ALERT_TRIGGER_DIR to point at
# it), so the QA pack carries the fire→resolve evidence for each alert.
# ---------------------------------------------------------------------------
echo "[package-m3] step 5f — alert-trigger logs"
if [ -n "${ALERT_TRIGGER_DIR:-}" ] && [ -f "${ALERT_TRIGGER_DIR%/}/timeline.md" ]; then
  cp -r "${ALERT_TRIGGER_DIR%/}" "$OUT_DIR/alert-trigger"
  echo "[package-m3]   alert-trigger bundle folded in"
  ALERT_TRIGGER_MD=$'A run of `scripts/monitoring/trigger-alert-demo.sh` fired each alert through the live pipeline and captured the transitions. The full timeline and per-alert snapshots (Prometheus state + feeder `alert_log` + dashboard PNGs) are under [`alert-trigger/`](alert-trigger/timeline.md).\n\n'"$(cat "${ALERT_TRIGGER_DIR%/}/timeline.md")"
else
  echo "[package-m3]   no ALERT_TRIGGER_DIR provided — noting how to generate it" >&2
  ALERT_TRIGGER_MD=$'_No alert-trigger run was folded into this pack. Generate one against the live monitoring stack and re-run with `ALERT_TRIGGER_DIR` pointing at its output:_\n\n```sh\ncd offchain && make up MONITORING=1\ncd feeder && scripts/monitoring/trigger-alert-demo.sh        # fires the safe set\n# then: ALERT_TRIGGER_DIR=docs/milestones/evidence/alert-trigger-<net>-<stamp> make evidence3\n```\n\n_Each alert is fired by pushing its metric to the Pushgateway; the real rule fires and flows through Prometheus → Alertmanager → the feeder webhook → `alert_log`._'
fi

# ---------------------------------------------------------------------------
# Step 6 — write SUMMARY.json + milestone-3-preview-evidence.md.
# ---------------------------------------------------------------------------
echo "[package-m3] step 6/6 — generating SUMMARY.json + evidence markdown"
...
# Milestone 3 ${NETWORK_DISPLAY} Evidence
...
echo "[package-m3] done."
echo "[package-m3] open: $OUT_DIR/$MD_FILE"
