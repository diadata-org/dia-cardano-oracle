...
export async function createMetrics(options: MetricsOptions = {}): Promise<FeederMetrics> {
  ...
  return {
    ...
    transactionsTotal: counter(
      "transactions_total",
      "Cardano transactions, counted once per tx (not per symbol), by outcome. The `wallet` label is the signer wallet that built the tx (pool id; empty for a dry-run or pre-pool build).",
      ["client_id", "customer_id", "outcome", "wallet"],
    ),
    transactionPairs: histogram(
      "transaction_pairs",
      "Pairs per Cardano transaction (batch size), one observation per tx. The `wallet` label is the signer wallet that built the tx.",
      ["client_id", "customer_id", "outcome", "wallet"],
      PAIRS_PER_TX_BUCKETS,
    ),
    ...
    txProcessingToSubmissionSeconds: histogram(
      ...
      "Tx-level seconds from processing start to submission, one observation per confirmed tx. The `wallet` label is the signer wallet that built the tx.",
      ["client_id", "customer_id", "wallet"],
      LATENCY_SECONDS_BUCKETS,
    ),
    txSubmissionToConfirmationSeconds: histogram(
      "tx_submission_to_confirmation_seconds",
      "Tx-level seconds from submission to confirmation, one observation per confirmed tx. The `wallet` label is the signer wallet that built the tx.",
      ["client_id", "customer_id", "wallet"],
      LATENCY_SECONDS_BUCKETS,
    ),
    txEndToEndSeconds: histogram(
      "tx_end_to_end_seconds",
      "Tx-level end-to-end seconds (processing start to confirmation), one observation per confirmed tx. The `wallet` label is the signer wallet that built the tx.",
      ["client_id", "customer_id", "wallet"],
      LATENCY_SECONDS_BUCKETS,
    ),
    ...
    cardanoWalletUsableUtxos: gauge(
      ...
      "Per signer-wallet count of pure-ADA UTxOs at or above the collateral floor — how many disjoint inputs the arbiter can hand to concurrent lanes. Below `wallet_shape.min_usable_utxos` (with a splittable UTxO present) the wallet is split for more parallelism. Labels: `wallet` (pool id), `role` (`main`/`pool`).",
      ["wallet", "role"],
      true,
    ),
    cardanoWalletReservations: gauge(
      "cardano_wallet_reservations",
      "Per signer-wallet count of active arbiter reservations (lanes currently building against this wallet). Labels: `wallet` (pool id), `role` (`main`/`pool`).",
      ["wallet", "role"],
      true,
    ),
