...
- The three global scripts (`config_state`, `update_coordinator`, `payment_hook`) are compiled once and deployed once; their hashes are chain-wide constants.
...
### 1.4 Reference-script deployment (global, step 6 above)

```mermaid
flowchart LR
  Admin([DIA Admin Wallet]):::wallet

  Admin --> TX((Publish global<br/>reference scripts)):::tx

  TX --> RSC[Reference-script UTxO<br/>script: config_state<br/>at always-fail addr]:::script
  TX --> RSK[Reference-script UTxO<br/>script: update_coordinator<br/>at always-fail addr]:::script
  TX --> RSH[Reference-script UTxO<br/>script: payment_hook<br/>at always-fail addr]:::script
...
- **Frequency:** once per chain, before **5.1 Config bootstrap**.
- **Inputs:** admin wallet UTxOs funding the min-UTxO of each reference-script output.
- **Outputs:** three reference-script UTxOs, each carrying the compiled script binary in its `reference_script` field. No datum, no mint, no redeemer.
- **Address:** an always-fail address, so the binary is effectively immutable. The specific address is a deployment choice and not part of the protocol.
- **Note:** can be split into three separate txs if the admin wallet cannot cover all three min-UTxOs in one input set; the protocol does not require a single-tx deployment.
...
flowchart LR
  ...
  TX --> RSR[Reference-script UTxO<br/>script: receiver of client X<br/>at always-fail addr]:::script
  TX --> RSP[Reference-script UTxO<br/>script: pair_state of client X<br/>at always-fail addr]:::script
  TX --> Change([Admin change]):::wallet
